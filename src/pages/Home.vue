...
export default {
  ...
  created(){
    this.$ajax.get("http://yiban.scau.edu.cn/renwen/public/user/info")
    .then((res)=>{
    ...
    .catch((err)=>{
      window.location.href="http://yiban.scau.edu.cn/renwen/public/login";
    })
  },
  methods:{
    logout(){
      this.$ajax.get("http://yiban.scau.edu.cn/renwen/public/logout")
      .then((res)=>{
        localStorage.clear();
        window.location.href="http://yiban.scau.edu.cn/renwen/public/login";
      })
